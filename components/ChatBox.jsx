    ...
    >
      <div className="chat-info">
        ...
        <div className="flex flex-col gap-1">
          {chat?.isGroup ? (
            <p className="text-base-bold">{chat?.name || 'Group'}</p>
          ) : (
      ...
      <div className="flex flex-col items-end">
        <p className="text-small-medium text-nowrap text-grey-3">
          {!lastMessage
        ...
        {chat?.messages?.some(message => 
          ...
          <p className="read">
            {chat?.messages?.filter(message => 
              !message.seenBy.some(member => member._id === currentUser._id)).length}
          </p>
        )}
      </div>
    </div>
